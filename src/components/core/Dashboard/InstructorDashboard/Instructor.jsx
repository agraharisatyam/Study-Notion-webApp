...
export default function Instructor() {
  ...
  // ✅ Ensure initial states are arrays to avoid `undefined` errors
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        console.error("Error in Instructor Dashboard useEffect:", error);
      } finally {
        setLoading(false);
      }
    })();
  ...
  const totalAmount = instructorData?.reduce(
    (acc, curr) => acc + (curr.totalAmountGenerated || 0), 0
  );

  const totalStudents = instructorData?.reduce(
    (acc, curr) => acc + (curr.totalStudentsEnrolled || 0), 0
  );
